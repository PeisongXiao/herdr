...
use crate::api::schema::{
    ...
    SubscriptionEventKind, TabCreateParams,
};
...
pub(super) fn wait_for_agent_status_change(
    pane_id: String,
    agent_status: AgentStatus,
    timeout_ms: Option<u64>,
) -> std::io::Result<i32> {
    wait_for_agent_statuses(pane_id, vec![agent_status], timeout_ms)
}

pub(super) fn wait_for_agent_statuses(
    pane_id: String,
    ...
    if agent_statuses.is_empty() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "agent status wait requires at least one status",
        ));
    }
    let match_event = if agent_statuses.len() == 1 {
        EventMatch::PaneAgentStatusChanged {
            pane_id,
            agent_status: agent_statuses[0],
        }
    } else {
