...
- The hidden desktop MCP bridge (`herdr mcp serve`) and its durable message queue (`herdr desktop enqueue`) are now available on Linux in addition to macOS, and any MCP-capable coding agent can register the STDIO bridge, not just Codex desktop. The bridge exposes the same ten tools on both platforms; queue format, limits, and the capability model are unchanged.
