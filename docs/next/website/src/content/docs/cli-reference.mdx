...
`herdr remote-resume` retries re-acquiring remote panes that a previous graceful server stop handed off under `[remote] auto_remote_handoff`. Automatic re-acquire at server start is non-interactive, so panes on hosts that need a password or MFA stay pending; `remote-resume` authenticates in the foreground like a normal `ssh` login and resumes them. `--list` shows pending panes and their last errors without resuming; `--peer` limits the retry to one peer.
