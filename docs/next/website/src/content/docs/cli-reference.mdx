...
`--output PATH` to write that document to a file. `api bridge` reads an initial
JSON request from stdin, forwards it to the local full API socket, and streams
the response to stdout. Herdr invokes it through authenticated SSH commands for
federation setup and remote-agent status; it does not publish that socket over
the peer link.
...
`herdr server` runs the headless server explicitly. Use it for supervised or service-style setups. `ensure` starts the background server only when it is not already running. `server stop` warns when remotely presented panes will be terminated; run `herdr remote-handoff` inside a pane first when it must continue on its host, or enable `[remote] auto_remote_handoff` to hand every delegated pane back automatically on stop. `reload-config` applies reloadable settings without restarting panes. `agent-manifests` shows the active bundled or local agent detection manifest sources and warnings; pass `--json` for the raw status response. `reload-agent-manifests` reloads manifests into the running server after local override edits.
...
Session deletion refuses to discard parked-terminal recovery tickets. `--force` is destructive: it deletes those tickets, reports how many remote terminals may remain hidden as orphans, and relies on the later orphan review to retain, terminate, or promote them.
...
Agent targets can be terminal IDs, unique agent names, detected or reported agent labels, `peer::target`, or legacy pane IDs. Names and labels are agent identities. Terminal IDs and legacy pane IDs are low-level escape hatches.
...
`agent explain` asks the running server to classify the same bottom-buffer detection snapshot used by screen detection, so live output reflects the server's active manifest cache. Because this uses the `agent.explain` socket method, restart or hand off to an updated server after upgrading Herdr before using live explain. Use `--file PATH --agent LABEL` to explain a saved fixture locally instead. The default output shows the agent, final state, manifest source and version, matched rule with its region evidence, and any fallback, skip, or warning reasons. Add `--verbose` for visible evidence flags and the full evaluated-rules list with matcher and region evidence. Add `--json` for issue reports or tests.

`agent start --ssh TARGET` is Unix-only and requires a compatible `peisongxiao/herdr` build to be installed remotely already. It finds that binary on the remote `PATH` or at `~/.local/bin/herdr`, starts its daemon, starts the agent, and delegates its pane into the local workspace. It never provisions, installs, or updates the remote binary. `--agent AGENT` selects the official integration to prepare; without it, Herdr tries to recognize `argv[0]`. Pass `--no-remote-integration` to skip setup. The start explicitly selects remote Herdr session `default` unless `--ssh-session NAME` selects a named session, so inherited or SSH-accepted Herdr session and socket variables cannot redirect it. Put custom SSH connection options in SSH config. The CLI checks effective SSH config before opening managed SSH, and the server enforces the same interactive-only boundary for direct API starts.

The delegated agent pane has one presentation owner and is removed from the remote workspace while local Herdr presents it. Closing that owner pane, stopping the owner Herdr server, or losing SSH terminates the remote pane and agent; it does not restore as a shell or remain available for later reattach. Run `herdr remote-handoff` inside it first when the pane should continue in a new workspace on the remote host.

The SSH start also registers a runtime peer. From the initiating server, peer targets support `agent list`, `get`, `read`, `explain`, `send`, `rename`, `wait`, `attach`, and `start`. The restricted reverse link supports `ping`, `list`, `get`, and the one-way presentation-activation acknowledgement; it cannot explain origin agents or read terminal or detection-preview data, inject input, or launch origin processes. `agent focus` does not change focus on another server; use `agent attach peer::target`. A peer attach must run inside the pane that will own the presentation. It refuses an already presented pane unless `--takeover` is explicit; takeover disconnects the prior owner rather than duplicating the pane. A pane with a deeper active remote presentation must unwind that deeper hop before takeover. `agent start --peer PEER` starts an agent on a peer reached in the SSH trust direction, without presenting its pane locally, and does not accept `--focus`.

Inside a Unix Herdr pane, a destination-only interactive `ssh TARGET` is routed through `herdr ssh`. Common connection and authentication options such as `-p`, `-i`, `-J`, `-F`, `-A`, `-4`/`-6`, and compatible `-o` settings are preserved. TTY flags are accepted, and the final attach always requests a TTY. OpenSSH performs host-key and credential prompts in the foreground, then Herdr reuses its control connection to find or start the remote Herdr daemon, verify matching protocol plus `peer_federation` and `remote_presentation` capabilities, register the restricted peer-agent bridge, and claim a new remote shell. The delegated shell explicitly belongs to remote Herdr session `default` and inherits the SSH session's directory and environment; inherited or accepted Herdr routing variables cannot select another session. Plain `ssh` does not select a named Herdr session; an explicit remote agent start may select one with `--ssh-session NAME`.

Commands after the destination, `-L`/`-R`/`-D`/`-W` forwarding, `-w` tunnels, `-N`, `-f`, `-s`, `-T`, equivalent semantic `-o` forms, and unrecognized option shapes fall through to the real `ssh`. Herdr also checks effective SSH config: remote-command, noninteractive or sessionless, local-command, forwarding, and tunnel semantics selected by `Host`, `Match`, or included config cause the original raw argv to fall through unchanged. A missing or incompatible remote Herdr, a failed capability check, a local API error, or another managed-setup failure prints a warning and reruns the original raw argv as ordinary SSH. `scp` and `sftp` are unchanged. Set `[remote].ssh_integration = false` or run `HERDR_SSH_BYPASS=1 ssh TARGET` to bypass the shim. Explicit `agent start --ssh` and API starts fail instead of taking this pane-only fallback.

Managed peer setup requires remote Unix-socket forwarding to be allowed by the SSH daemon and any applicable `authorized_keys` options. If it is denied, Herdr retains the actionable `remote peer bridge failed` diagnostic that identifies remote Unix-socket forwarding and server or authorized-key restrictions. Explicit agent and API starts stop instead of claiming one-way or partial awareness; the pane shim then runs the original SSH argv. See [Persistence and remote access](/docs/persistence-remote/) for details.

Managed pane SSH is exclusive and destructive. The remote daemon keeps the process and PTY, but removes the delegated pane from its workspaces while the initiating machine presents it. When SSH disconnects, its owner pane closes, or the owning Herdr server stops, the remote daemon terminates the delegated pane. A rejected final attach abandons its prepared claim and leaves the local owner pane intact instead of cascading a close. Nested supported SSH carries the same owner through each successful hop; disconnect unwinds the active chain.

`herdr remote-handoff` takes no arguments and must be run inside the currently delegated pane containing your cursor. It moves only that live pane into a new workspace on its actual host, ends the managed attach, and closes the old owner pane. In a nested SSH chain it hands off the innermost pane. Herdr rejects an intermediate handoff while that pane owns or is preparing a deeper remote presentation. It is unrelated to the server-replacement `--handoff` options. Use `herdr agent attach peer::target --takeover` to replace another presentation owner explicitly; Herdr never takes over implicitly.
...
`terminal shell` creates a server-owned shell tab from the caller's current
directory and environment, removes Herdr's internal routing variables, and
attaches to it. Managed pane SSH instead prepares an exclusive terminal
delegation and commits it through `terminal attach`.
...
`terminal parked` is a local administrative view of live PTYs that are intentionally hidden from this server's workspaces. `promote` moves one into a visible workspace in the current session; `terminate` stops it permanently.
...
Peers are runtime links created by `ssh TARGET` inside a Herdr pane or by `herdr agent start --ssh`. `peer list` shows their generated ids, `peer health` refreshes reachability, and `peer unregister` removes all managed bridges for that peer. A plain SSH bridge closes with its foreground attach; while it is open, the wrapper renews a 15-second server lease so abrupt wrapper termination is cleaned up automatically. The incoming reverse registration has its own 15-second lease, renewed only after a successful metadata refresh, so origin crashes or network loss remove ghost peers. A peer-agent attachment uses its own 15-second owner-presentation lease, renewed by successful remote metadata snapshots, so closing or crashing that attachment does not consume or tear down the shared peer bridge. Every bridge conditionally unregisters only the registration for its own forwarded socket, preserving concurrent bridges. Peer-aware agent commands accept `peer::target`. Links are not durable machine pairings and must be recreated after the owning Herdr server restarts. Losing an owning link terminates its delegated pane unless `remote-handoff` has already moved that pane into a workspace on its host. Active links, active presentation leases, and detached bridge cleanup block server live handoff; after unregistering, wait for cleanup to finish before retrying.

`herdr remote-resume` is the interactive-authentication and retry fallback for remote panes that a previous graceful server stop handed off under `[remote] auto_remote_handoff`. Every existing ticket is tried automatically at server start regardless of the setting's current value. Startup recovery is non-interactive, so panes on hosts that need a password or MFA stay pending; `remote-resume` authenticates in the foreground like a normal `ssh` login and resumes them. `--list` shows pending panes and their last errors without resuming; `--peer` limits the retry to one peer. A host-confirmed ended terminal is removed from the pending list automatically.
...
Configure a supported MCP client with a fixed `herdr` server entry:

```bash
herdr mcp install codex
herdr mcp install claude
herdr mcp install hermes
herdr mcp install openclaw
herdr mcp install codex --full-control
herdr mcp status
herdr mcp status codex
herdr mcp uninstall codex
```

Supported client IDs are `codex`, `claude` (with `claude-code` as an alias),
`hermes`, and `openclaw`. `mcp install` defaults to the restricted tool set.
Use `--full-control` only when the client should have the same interaction
surface as a person at the Herdr UI. `mcp status [client]` inspects configured
entries, and `mcp uninstall <client>` removes only the fixed `herdr` entry.

To bind the installed server to a named Herdr session, select it when
installing:

```bash
herdr --session work mcp install codex
```

The generated entry carries the selected `HERDR_SESSION`, so all MCP tools
target that session namespace. Installation only changes client configuration:
it does not start an MCP connection. Restart or reload the client after a
change. The sidebar indicator stays gray until the client starts the configured
stdio bridge, then turns green while at least one connection is active.

OpenCode and unrecognized client IDs exit with `Currently unsupported`. Use the
manual `herdr mcp serve [--full-control]` setup in the
[MCP bridge documentation](/docs/socket-api/#mcp-bridge) when automatic setup
is unavailable.
