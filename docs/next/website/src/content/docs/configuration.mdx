...
On Unix, `ssh_integration` puts a small `ssh` shim first on each Herdr pane's `PATH`. Destination-only interactive `ssh TARGET` invocations authenticate with normal foreground OpenSSH, find or start a compatible remote Herdr daemon, and exclusively delegate a shell from its default session into the current pane. The remote host must already have Herdr with the required peer-federation and remote-presentation capabilities; setup failures warn and fall back to ordinary SSH. Remote-command, noninteractive or sessionless, forwarding, tunnel, and local-command semantics also fall through to the real `ssh` unchanged. Herdr applies that boundary to effective SSH config as well as literal arguments, so matching `Host`, `Match`, or included config is preserved rather than overridden to force integration.

For newly created macOS login-zsh panes, including the default `shell_mode = "auto"` path, Herdr uses a private one-shot startup hook. It restores the user's original `ZDOTDIR` and startup files, then re-prepends the shim and runs `rehash` at the first prompt after `path_helper` and other login setup have finished. The hook removes itself after that prompt. User aliases and functions still take their normal precedence, and an explicit absolute path such as `/usr/bin/ssh` bypasses the shim. Existing panes keep their current environment until they are recreated.

This setting is independent of `manage_ssh_config`: pane SSH integration always uses its own temporary OpenSSH control connection when an interactive terminal is available, while `manage_ssh_config` controls only `herdr --remote`. Set `ssh_integration = false` to leave pane `ssh` untouched, or use `HERDR_SSH_BYPASS=1 ssh TARGET` for one command. See [Persistence and remote access](/docs/persistence-remote/) for supported forms, remote Herdr discovery, and peer-link behavior.

With `auto_remote_handoff = true`, a graceful server stop hands every delegated remote pane back to its host instead of terminating it, and the next server start re-acquires the pane into the workspace and tab it came from. The setting controls creation of new handoffs; every existing recovery ticket is reconciled on startup even if the setting is later disabled. Automatic re-acquire is non-interactive, so it needs key- or agent-based SSH authentication; hosts that require a password or MFA stay recorded and can be resumed with `herdr remote-resume`. Ungraceful stops (crash, power loss, network failure) still terminate remote panes, and nested SSH chains deeper than one hop are not handed off. See [Persistence and remote access](/docs/persistence-remote/#automatic-handoff-on-server-stop) for the full lifecycle.

You can also toggle this experimental behavior from Settings > Experiments > automatic remote handoff. The toggle updates `[remote].auto_remote_handoff`.
