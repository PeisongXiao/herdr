...
bridge が公開するのは上記の 10 tool だけです。health、snapshot、pane read、process inspection は read-only です。launch/send/interrupt/stop はこの MCP が起動して登録した CLI だけに適用され、任意の既存 pane は inspect できますが mutate できません。raw API forwarding、Herdr server start/stop/restart、subscription、focus/takeover、integration、plugin、client 自身の task lifecycle は公開しません。MCP child は client が所有し、stdin を閉じると bridge は終了しますが Herdr server は変更しません。
...
これは同じ OS user に対する capability と product-policy の境界であり、local account に対する hardened authorization ではありません。queue directory と opaque channel capability は偶発的な cross-channel access を防ぎ、MCP mutation tool は登録 pane identity を再検証しますが、既存の Herdr UI、CLI、full local socket API は同じ user に通常の authority を保ちます。desktop agent を意図上唯一の manual entry point とする用途には適しますが、敵対的な local process の sandbox ではありません。
