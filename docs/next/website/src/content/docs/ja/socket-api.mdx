...
## macOS Codex デスクトップ MCP

macOS では、Codex デスクトップが Herdr の local STDIO MCP bridge を child process として起動できます。2 つの entry point は Herdr の help、completion、menu、integration list から意図的に隠されています。`herdr --session NAME mcp serve` が bridge を実行し、その bridge が起動した CLI は `herdr desktop enqueue` で desktop inspection 待ちの message を追加できます。Codex 側では configured MCP server が Settings と `/mcp` UI に表示されます。

Herdr binary の absolute path を使い、Codex の shared configuration に手動登録します:
...
bridge が公開するのはこの 10 tool だけです。health、snapshot、pane read、process inspection は read-only です。launch/send/interrupt/stop はこの MCP が起動して登録した CLI だけに適用され、任意の既存 pane は inspect できますが mutate できません。raw API forwarding、Herdr server start/stop/restart、subscription、focus/takeover、integration、plugin、Codex task lifecycle は公開しません。MCP child は Codex が所有し、stdin を閉じると bridge は終了しますが Herdr server は変更しません。

起動された CLI は予約済み `HERDR_DESKTOP_*` 環境変数から capability-scoped local queue を受け取ります。`herdr desktop enqueue` は bounded UTF-8 message を追加するだけで、Herdr API client を持たず、自身の process/pane/server/MCP lifecycle を制御できません。Codex、MCP child、Herdr が停止中でも message は永続化されます。`herdr_drain_messages` は削除せず FIFO batch を原子的に lease し、desktop が処理後に `herdr_ack_messages` でその lease を明示的かつ idempotent に削除します。ack されない lease は期限後 queue に戻るため、inspection で message を暗黙に失いません。
...
これは同じ OS user に対する capability と product-policy の境界であり、local account に対する hardened authorization ではありません。queue directory と opaque channel capability は偶発的な cross-channel access を防ぎ、MCP mutation tool は登録 pane identity を再検証しますが、既存の Herdr UI、CLI、full local socket API は同じ user に通常の authority を保ちます。Codex desktop を意図上唯一の manual entry point とする用途には適しますが、敵対的な local process の sandbox ではありません。
