...
対応するクライアントでは、自動セットアップを優先してください。
...
インストーラーは Codex、Claude Code、Hermes、OpenClaw に対応します。
`--full-control` を明示しない限り、固定名 `herdr` のエントリを制限モードで
書き込みます。OpenCode とその他のクライアント ID は
`Currently unsupported` を表示するため、下記の手動設定を使用してください。
エントリをインストールしただけでは接続は作成されず、MCP 表示も緑色には
なりません。クライアントが設定済みの stdio ブリッジを開始した時点で接続中に
なります。
...
`server.agent_manifests` は、ルールをリロードせずに、アクティブな bundled/local エージェント検出マニフェストのソースを返します:
...
optional な `active_version` と `warning` は利用できないときに省略されます。`server.reload_agent_manifests` は、メモリ内のルールキャッシュをリロードした後、同じ `manifests` の項目形式を持つ `agent_manifest_reload` を返します。
