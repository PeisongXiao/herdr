---
title: セッション状態と復元
description: Herdr が何をライブで維持し、再起動後に何を復元し、履歴から何をリプレイし、エージェントインテグレーションで何を resume するかを理解します。
---
...
`./install-local.sh` の再実行は installed binary を置き換えますが、すでに動作中の server process は置き換えません。default または named session を停止して再起動するまでは既存 pane がそのまま動き、再起動時には上の server restart 行が適用されます。

これらの永続化保証は、現在の Herdr セッションが所有するペインに適用されます。managed `ssh` または `agent start --ssh` で委譲されたペインには、より厳しいライフサイクルがあります。SSH の切断、owner ペインのクローズ、owner サーバーの停止はいずれもリモートペインとプロセスを終了します。残す必要がある場合は、先にそのペイン内で `herdr remote-handoff` を実行し、実際のホスト上の新しいワークスペースへ戻してください。
...
ライブハンドオフは、稼働中の Herdr server を置き換える必要がある remote attach で利用できます。古い server に live pane を新しい server へ移すよう依頼し、server replacement をまたいで pane process が動き続けられるようにします。

これはスナップショット復元、ペイン履歴リプレイ、エージェントネイティブのセッション復元とは異なります。ハンドオフは現在のプロセスを生かし続けようとします。他の経路は、古いサーバーが停止した後に状態を再構築します。

これは `herdr remote-handoff` とも異なります。後者はカーソル位置だけを対象にし、現在委譲されている SSH ペイン 1 つを、その terminal を実行しているマシン上の新しいワークスペースへ戻します。サーバーは置き換えません。
...
素の `herdr --remote workbox` は default で通常の restart/stop flow を使います。この fork に updater-driven handoff はありません。source upgrade では `./install-local.sh` と明示的な session restart を使います。
