...
`auto_remote_handoff = true` にすると、正常停止したサーバーは委譲されたリモートペインを終了する代わりにホストへ引き渡し、次回のサーバー起動時に元のワークスペースとタブへ再取得します。自動再取得は非対話的なので、鍵またはエージェントによる SSH 認証が必要です。パスワードや MFA が必要なホストは記録されたまま残り、`herdr remote-resume` で再取得できます。正常ではない停止（クラッシュ、電源喪失、ネットワーク障害）では従来どおりリモートペインは終了し、1 ホップを超える入れ子の SSH チェーンは引き渡しされません。ライフサイクル全体は[永続化とリモートアクセス](/ja/docs/persistence-remote/#automatic-handoff-on-server-stop)を参照してください。
