...
Unix では、`ssh_integration` により各 Herdr ペインの `PATH` の先頭に小さな `ssh` shim が置かれます。接続先だけを指定する対話的な `ssh TARGET` は通常の OpenSSH でフォアグラウンド認証を行い、互換性のあるリモート Herdr デーモンを検出または起動し、リモートの default セッションのシェルを現在のペインへ排他的に委譲します。リモート側には peer federation と remote presentation を備えた互換 Herdr が事前に必要です。セットアップ失敗時は警告して通常の SSH にフォールバックします。リモートコマンド、非対話/セッションなし、フォワーディング、トンネル、ローカルコマンドの意味を持つ接続は、元の引数のまま実際の `ssh` に渡されます。Herdr は `Host`、`Match`、`Include` を含む有効な SSH 設定にも同じ境界を適用し、統合を強制するために設定を上書きしません。
