...
`agent explain` は、スクリーン検出が使うのと同じ下部バッファの検出スナップショットの分類を実行中のサーバーに依頼します。そのためライブの出力はサーバーのアクティブなマニフェストキャッシュを反映します。これは `agent.explain` ソケットメソッドを使うので、Herdr のアップグレード後にライブ explain を使う前に、サーバーを再起動するか更新済みサーバーへハンドオフしてください。保存済みフィクスチャをローカルで説明するには `--file PATH --agent LABEL` を使います。デフォルトの出力には、エージェント、最終状態、マニフェストのソースとバージョン、リージョンの証拠付きでマッチしたルール、そしてフォールバック・スキップ・警告の理由が表示されます。`--verbose` を付けると、可視の証拠フラグとマッチャー/リージョンの証拠付きの評価済みルール全リストが表示されます。issue の報告やテストには `--json` を付けてください。

`agent start --ssh TARGET` は Unix-only で、互換性のある `peisongxiao/herdr` build が remote host に事前 install されている必要があります。remote `PATH` または `~/.local/bin/herdr` で binary を探し、その daemon と agent を起動して pane を local workspace へ委譲します。remote binary の provision/install/update は行いません。`--agent AGENT` が公式 integration を選び、省略時は `argv[0]` を認識します。setup を省くには `--no-remote-integration` を使います。default remote session は明示的に `default` で、named session は `--ssh-session NAME` だけが選びます。SSH option は config に置き、CLI/server は managed SSH 前に対話意味を変える有効 config を拒否します。
...
`herdr remote-resume` は、前回の正常停止が `[remote] auto_remote_handoff` でホストへ引き渡したリモートペイン向けの対話認証および再試行用 fallback です。現在の設定値にかかわらず、既存 ticket はサーバー起動時に自動で再取得を試みます。自動再取得は非対話的なので、パスワードや MFA が必要なホストのペインは pending のまま残り、`remote-resume` が通常の `ssh` ログインのようにフォアグラウンドで認証して再取得します。`--list` は pending のペインと最後のエラーを再取得せずに表示し、`--peer` は再試行を 1 peer に限定します。host が終了済みと確認した terminal は pending list から自動的に削除されます。
...
対応する MCP クライアントのユーザー設定に、固定名 `herdr` のサーバーエントリを設定します。
...
対応するクライアント ID は `codex`、`claude`（別名 `claude-code`）、
`hermes`、`openclaw` です。`mcp install` は既定で制限付きツールセットを
設定します。Herdr UI を操作する人と同じ操作範囲が必要な場合だけ
`--full-control` を指定してください。`mcp status [client]` は設定済み
エントリを確認し、`mcp uninstall <client>` は固定名 `herdr` のエントリ
だけを削除します。

インストールするサーバーを名前付き Herdr セッションに固定するには、
インストール時にそのセッションを選択します。

```bash
herdr --session work mcp install codex
```

生成されるエントリには選択した `HERDR_SESSION` が保持されるため、すべての
MCP ツールはそのセッションの名前空間を対象にします。インストールは
クライアント設定を変更するだけで、MCP 接続は開始しません。変更後は
クライアントを再起動または再読み込みしてください。サイドバーの表示は、
クライアントが設定済みの stdio ブリッジを開始するまでは灰色で、1 つ以上の
接続が有効な間だけ緑色になります。

OpenCode と未認識のクライアント ID は `Currently unsupported` を表示して
終了します。自動セットアップを利用できない場合は、
[MCP ブリッジのドキュメント](/docs/ja/socket-api/#mcp-bridge) にある
`herdr mcp serve [--full-control]` の手動設定を使用してください。
