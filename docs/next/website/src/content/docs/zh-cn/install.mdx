...
`--check` 会检查 Cargo 与 Rust 版本、编译并链接最小 C 程序、验证精确 Zig 版本,并在不构建 Herdr 的情况下报告平台、Zig 路径、隔离的构建缓存和安装目标。默认执行锁定的 release 构建,再原子替换 `$HOME/.local/bin/herdr`。构建产物按 checkout 路径隔离在 `${XDG_CACHE_HOME:-$HOME/.cache}/herdr/install-local` 下,避免其他 checkout 或 Docker bind mount 留下的本地库路径污染构建;显式 `CARGO_TARGET_DIR` 会覆盖它。用 `--prefix DIR` 或 `PREFIX` 改安装前缀,用 `--bin-dir DIR` 直接指定二进制目录,用 `--debug` 构建 debug 版本。`--clean-install` 是仅针对所选 profile 的显式破坏性 factory reset,该 flag 本身即授权 reset:release 清理 `herdr`,`--debug` 清理 `herdr-dev`。它会先 stage candidate,再停止检测到的 default 与 named-session server,quarantine 并删除该 profile 的 config/state tree(包括全部 session 与 remote recovery ticket),最后安装 candidate。由于停止 owning server 会中断安装,脚本拒绝从 Herdr 内运行。删除 recovery ticket 可能留下 hidden remote process。外部 agent/MCP/SSH 配置、project、另一 profile 与 build cache 都不会被删除。目标不在 `PATH` 时脚本会警告;直接 SSH federation 仍会发现默认的 `~/.local/bin/herdr`。
