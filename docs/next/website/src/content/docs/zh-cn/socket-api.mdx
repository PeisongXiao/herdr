...
在 macOS 和 Linux 上,任何支持 MCP 的编程智能体都可以把 Herdr 的本地 STDIO MCP bridge 作为子进程启动。两个入口都有意从 Herdr 的帮助、补全、菜单和集成列表中隐藏:`herdr --session NAME mcp serve` 运行 bridge,由该 bridge 启动的 CLI 则可用 `herdr desktop enqueue` 追加一条等待桌面检查的消息。已配置的 MCP 服务器仍会显示在客户端自己的设置界面中。
...
bridge 只公开以上十个工具。health、snapshot、pane read 和 process inspection 是只读操作;launch、send、interrupt、stop 只作用于由该 MCP 启动并登记的 CLI,任意现有窗格可以检查,但不能通过 bridge 修改。它不公开 raw API forwarding、Herdr server 的 start/stop/restart、订阅、focus/takeover、集成、插件或客户端自身的 task 生命周期。客户端拥有 MCP 子进程;关闭 stdin 会结束 bridge,但不会改变 Herdr server。
...
这是同一 OS 用户下的 capability 与产品策略边界,不是防御本地账号的强化授权机制。队列目录与不透明 channel capability 防止意外跨 channel 访问,MCP 写操作也会重新验证登记的 pane identity;但既有 Herdr UI、CLI 和完整本地 socket API 对同一用户仍保留正常权限。因此它适合把桌面智能体作为约定中的唯一人工入口,并不用于隔离恶意本地进程。
