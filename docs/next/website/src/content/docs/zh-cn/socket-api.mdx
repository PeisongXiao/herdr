...
在 macOS 上,Codex 桌面应用可以把 Herdr 的本地 STDIO MCP bridge 作为子进程启动。两个入口都有意从 Herdr 的帮助、补全、菜单和集成列表中隐藏:`herdr --session NAME mcp serve` 运行 bridge,由该 bridge 启动的 CLI 则可用 `herdr desktop enqueue` 追加一条等待桌面检查的消息。Codex 仍会在自己的 Settings 与 `/mcp` UI 中显示已配置的 MCP 服务器。

请在 Codex 的共享配置中手动注册,并使用 Herdr 二进制的绝对路径:
...
bridge 只公开以上十个工具。health、snapshot、pane read 和 process inspection 是只读操作;launch、send、interrupt、stop 只作用于由该 MCP 启动并登记的 CLI,任意现有窗格可以检查,但不能通过 bridge 修改。它不公开 raw API forwarding、Herdr server 的 start/stop/restart、订阅、focus/takeover、集成、插件或 Codex task 生命周期。Codex 拥有 MCP 子进程;关闭 stdin 会结束 bridge,但不会改变 Herdr server。

每个被启动的 CLI 会通过保留的 `HERDR_DESKTOP_*` 环境变量获得一个 capability-scoped 本地队列。`herdr desktop enqueue` 只能向该队列追加有界 UTF-8 消息;它没有 Herdr API client,不能控制自身进程、窗格、服务器或 MCP 生命周期。Codex、MCP 子进程或 Herdr 停止时,消息仍会持久保存。`herdr_drain_messages` 会原子 lease 一个 FIFO 批次而不是删除消息;桌面处理完成后必须调用 `herdr_ack_messages` 显式且幂等地删除该 lease。未 ack 的 lease 过期后重新回到队列,因此检查过程不会静默丢消息。
...
这是同一 OS 用户下的 capability 与产品策略边界,不是防御本地账号的强化授权机制。队列目录与不透明 channel capability 防止意外跨 channel 访问,MCP 写操作也会重新验证登记的 pane identity;但既有 Herdr UI、CLI 和完整本地 socket API 对同一用户仍保留正常权限。因此它适合把 Codex 桌面作为约定中的唯一人工入口,并不用于隔离恶意本地进程。
