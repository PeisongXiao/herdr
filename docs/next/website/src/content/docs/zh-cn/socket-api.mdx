...
安装器支持 Codex、Claude Code、Hermes 和 OpenClaw。除非显式指定
`--full-control`，否则它会以受限模式写入固定名称为 `herdr` 的条目。
OpenCode 和其他客户端 ID 会输出 `Currently unsupported`，请改用下方的手动
配置。安装条目本身不会建立连接，也不会让 MCP 指示器变绿；只有客户端启动
已配置的 stdio 桥后才会显示为已连接。
...
`server.agent_manifests` 返回生效的 bundled/local 智能体检测清单来源,不重载规则:
...
可选的 `active_version` 与 `warning` 字段在不可用时被省略。`server.reload_agent_manifests` 在重载内存中的规则缓存后,返回带相同 `manifests` 条目结构的 `agent_manifest_reload`。
