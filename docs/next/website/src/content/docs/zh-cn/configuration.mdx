...
在 Unix 上,`ssh_integration` 会把一个轻量的 `ssh` shim 放到每个 Herdr 窗格的 `PATH` 前面。仅含目标地址的交互式 `ssh TARGET` 会继续由前台 OpenSSH 处理认证,随后查找或启动兼容的远程 Herdr 守护进程,并把远程默认会话中的 shell 独占委派到当前窗格。远程主机必须已经安装支持 peer federation 与 remote presentation 的兼容 Herdr;设置失败时会警告并按原始参数回退到普通 SSH。远程命令、非交互或无会话模式、转发、隧道和本地命令语义始终原样交给真实 `ssh`;Herdr 也会检查由 `Host`、`Match` 和 `Include` 得到的有效 SSH 配置,不会为了强制集成而覆盖它。
