...
在 Unix 上,`ssh_integration` 会把一个轻量的 `ssh` shim 放到每个 Herdr 窗格的 `PATH` 前面。仅含目标地址的交互式 `ssh TARGET` 会继续由前台 OpenSSH 处理认证,随后查找或启动兼容的远程 Herdr 守护进程,并把远程默认会话中的 shell 独占委派到当前窗格。远程主机必须已经安装支持 peer federation 与 remote presentation 的兼容 `peisongxiao/herdr` build;Herdr 不会 provision 它。设置失败时会警告并按原始参数回退到普通 SSH。远程命令、非交互或无会话模式、转发、隧道和本地命令语义始终原样交给真实 `ssh`;Herdr 也会检查由 `Host`、`Match` 和 `Include` 得到的有效 SSH 配置,不会为了强制集成而覆盖它。

对于新建的 macOS login-zsh 窗格,包括默认的 `shell_mode = "auto"`,Herdr 使用一次性的私有启动钩子。它先恢复用户原本的 `ZDOTDIR` 和启动文件,等 `path_helper` 等登录初始化结束后,在第一个提示符重新把 shim 放到 `PATH` 前面并执行 `rehash`,随后自我移除。用户别名和函数仍按正常优先级工作,`/usr/bin/ssh` 这样的绝对路径会绕过 shim。已经运行的窗格要在重建后才获得新环境。

此设置与 `manage_ssh_config` 相互独立: 窗格 SSH 集成始终使用自己的临时 OpenSSH 控制连接,而 `manage_ssh_config` 只控制 `herdr --remote`。设置 `ssh_integration = false` 可完全保留原始窗格 `ssh`,单次绕过则使用 `HERDR_SSH_BYPASS=1 ssh TARGET`。支持的命令形式、远程发现和 peer 链接边界见[持久化与远程访问](/zh-cn/docs/persistence-remote/)。

设置 `auto_remote_handoff = true` 后,服务器正常停止时会把委派中的远程窗格交还给所在主机而不是终止它,并在下次服务器启动时把它重新取回原来的工作区与标签页。该设置只控制新 handoff 的创建;即使之后关闭设置,已有 recovery ticket 也会在启动时自动协调。自动取回是非交互式的,因此需要基于密钥或 agent 的 SSH 认证;需要密码或 MFA 的主机会保留在记录中,可用 `herdr remote-resume` 手动取回。非正常停止(崩溃、断电、网络故障)仍会终止远程窗格,且超过一跳的嵌套 SSH 链不会被交还。完整生命周期见[持久化与远程访问](/zh-cn/docs/persistence-remote/#automatic-handoff-on-server-stop)。

也可以在 Settings > Experiments > automatic remote handoff 中切换这一实验性行为。该开关会更新 `[remote].auto_remote_handoff`。
