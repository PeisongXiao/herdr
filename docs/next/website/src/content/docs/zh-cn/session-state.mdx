---
title: 会话状态与恢复
description: 理解 Herdr 保持哪些实时状态、重启后恢复什么、从历史回放什么,以及通过智能体集成恢复什么会话。
---
...
重新运行 `./install-local.sh` 会替换已安装的二进制,但不会替换已经运行的 server process。停止并重启 default 或 named session 前,现有窗格继续原样运行;重启时适用上面的服务器重启行。

这些持久化保证适用于由当前 Herdr 会话拥有的窗格。通过托管 `ssh` 或 `agent start --ssh` 委派的窗格具有更严格的生命周期: SSH 断开、所有者窗格关闭或所有者服务器停止都会终止远程窗格和进程。若要保留它,请先在该窗格内运行 `herdr remote-handoff`,把它放回真实主机的新工作区。
...
需要替换运行中 Herdr server 时,远程连接可以使用实时交接。它请求旧 server 把实时窗格转移给新 server,让窗格进程跨 server replacement 继续运行。

这与快照恢复、窗格历史回放和智能体原生会话恢复不同。交接尝试让当前进程活下去,其他路径则是在旧服务器已经停止之后重建状态。

它也不同于 `herdr remote-handoff`: 后者是仅针对光标所在窗格的命令,把一个当前委派的 SSH 窗格移回运行其终端的机器上的新工作区,并不会替换服务器。
...
普通的 `herdr --remote workbox` 默认使用常规的重启/停止流程。这个 fork 没有 updater-driven handoff;源码升级使用 `./install-local.sh` 并显式重启 session。
