...
`agent explain` 请求运行中的服务器对屏幕检测所用的同一份底部缓冲区检测快照进行分类,因此实时输出反映服务器生效的清单缓存。因为它使用 `agent.explain` socket 方法,升级 Herdr 后,请先重启或交接到更新后的服务器,再使用实时 explain。用 `--file PATH --agent LABEL` 可以改为在本地解释一份保存的样本。默认输出显示智能体、最终状态、清单来源和版本、匹配的规则及其区域证据,以及任何回退、跳过或警告原因。加 `--verbose` 可以看到可见证据标志,以及带匹配器和区域证据的完整已评估规则列表。提交问题报告或写测试时加 `--json`。

`agent start --ssh TARGET` 仅支持 Unix,并要求远程主机已经安装兼容的 `peisongxiao/herdr` build。它在远程 `PATH` 或 `~/.local/bin/herdr` 中寻找二进制,启动其 daemon,然后在远程启动智能体并把窗格委派到本地工作区;不会 provision、安装或更新远程二进制。`--agent AGENT` 选择要准备的官方集成,省略时尝试识别 `argv[0]`;用 `--no-remote-integration` 跳过准备。默认显式选择远程 `default` 会话,只有 `--ssh-session NAME` 选择命名会话,继承或 SSH 接受的 Herdr 路由变量不能改变它。连接选项放进 SSH 配置;CLI 与服务器都会在托管 SSH 前拒绝改变交互语义的有效配置。
...
`herdr remote-resume` 是为上一次正常停止时通过 `[remote] auto_remote_handoff` 交还给主机的远程窗格提供的交互认证与重试 fallback。无论当前设置值如何,已有 ticket 都会在服务器启动时自动尝试恢复。自动取回是非交互式的,因此需要密码或 MFA 的主机窗格会保持 pending;`remote-resume` 会像普通 `ssh` 登录一样在前台完成认证并取回。`--list` 只显示 pending 窗格及其最近的错误,`--peer` 将重试限制在单个 peer。经 host 确认已经结束的 terminal 会自动从 pending list 移除。
...
在受支持的 MCP 客户端用户配置中写入固定名称为 `herdr` 的服务器条目:
...
支持的客户端 ID 为 `codex`、`claude`（别名 `claude-code`）、`hermes` 和
`openclaw`。`mcp install` 默认配置受限工具集。只有在客户端需要与 Herdr UI
中的用户相同的操作范围时才使用 `--full-control`。`mcp status [client]`
检查已配置的条目，`mcp uninstall <client>` 只删除固定名称为 `herdr` 的条目。

若要把安装的服务器固定到命名 Herdr 会话，请在安装时选择该会话:

```bash
herdr --session work mcp install codex
```

生成的条目会保存所选的 `HERDR_SESSION`，因此所有 MCP 工具都以该会话的
命名空间为目标。安装只修改客户端配置，并不会建立 MCP 连接。更改后请重启或
重新加载客户端。侧边栏指示器会保持灰色，直到客户端启动已配置的 stdio 桥；
至少有一个连接处于活动状态时才会变为绿色。

OpenCode 和无法识别的客户端 ID 会输出 `Currently unsupported` 并退出。
自动设置不可用时，请使用
[MCP 桥文档](/docs/zh-cn/socket-api/#mcp-bridge) 中的
`herdr mcp serve [--full-control]` 手动设置。
