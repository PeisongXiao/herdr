...
这条 Unix-only 路径不会在普通 SSH PTY 中启动智能体。Herdr 会在远程 `PATH`、常见用户/系统、Homebrew、mise、Nix 路径以及 `~/.local/bin/herdr` 中寻找二进制,必要时启动守护进程,并验证协议、peer federation 和 remote presentation 能力;它不会安装或更新远程 Herdr。缺失或不兼容会产生明确错误。CLI 在托管 SSH 前会拒绝改变交互会话语义的有效 SSH 配置,服务器也对 SSH-backed agent/API 启动执行同样边界。
