...
By default Herdr prepares the matching official integration, using `--agent AGENT` when present or recognizing `argv[0]`. Use `--no-remote-integration` when hooks are already managed or the command is not a recognized agent. Herdr then starts the agent remotely, removes its pane from the remote workspace, and presents it in a local pane. This is exclusive delegation, not a durable mirror: only one machine's workspace owns the pane presentation at a time. Closing the local owner pane, stopping its Herdr server, or losing the connection terminates the delegated remote pane and agent. Run `herdr remote-handoff` inside the presented pane to keep it by moving it into a new workspace on the remote host. Remote agent starts explicitly select remote Herdr session `default` unless you pass `--ssh-session NAME`; inherited or SSH-accepted `HERDR_SESSION`, `HERDR_SOCKET_PATH`, and `HERDR_CLIENT_SOCKET_PATH` values cannot redirect that selection. Put custom ports, jump hosts, and identities in SSH config because `--ssh` accepts a target, not arbitrary SSH options.
