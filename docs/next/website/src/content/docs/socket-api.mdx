...
## macOS Codex desktop MCP

On macOS, Codex desktop can start Herdr's local STDIO MCP bridge as a child process. Both entry points are intentionally hidden from Herdr help, completions, menus, and integration lists: `herdr --session NAME mcp serve` runs the bridge, while a CLI launched by that bridge can use `herdr desktop enqueue` to append a message for later desktop inspection. Codex still shows the configured MCP server in its own Settings and `/mcp` UI.

Register it manually in the shared Codex configuration, using the absolute path to the Herdr binary:
...
The bridge exposes exactly those ten tools. Health, snapshot, pane read, and process inspection are read-only. Launch, send, interrupt, and stop apply only to CLIs launched and registered by this MCP process; arbitrary panes remain inspectable but cannot be mutated through the bridge. It never exposes raw API forwarding, Herdr server start/stop/restart, subscriptions, focus or takeover, integrations, plugins, or Codex task lifecycle. Codex owns the MCP child and closing its stdin ends it without changing the Herdr server.

Each launched CLI receives a capability-scoped, local queue through reserved `HERDR_DESKTOP_*` environment variables. `herdr desktop enqueue` can only append bounded UTF-8 messages to that queue; it has no Herdr API client and cannot control its process, pane, server, or MCP lifecycle. Messages persist while Codex, the MCP child, or Herdr is stopped. `herdr_drain_messages` atomically leases a FIFO batch instead of deleting it, and `herdr_ack_messages` explicitly and idempotently removes the leased messages after the desktop has handled them. An unacknowledged expired lease returns to the queue, so inspection does not silently lose messages.
