...
On macOS and Linux, any MCP-capable coding agent can start Herdr's local STDIO MCP bridge as a child process. Both entry points are intentionally hidden from Herdr help, completions, menus, and integration lists: `herdr --session NAME mcp serve` runs the bridge, while a CLI launched by that bridge can use `herdr desktop enqueue` to append a message for later desktop inspection. The client still shows the configured MCP server in its own settings UI.
...
The bridge exposes exactly the ten tools listed above. Health, snapshot, pane read, and process inspection are read-only. Launch, send, interrupt, and stop apply only to CLIs launched and registered by this MCP process; arbitrary panes remain inspectable but cannot be mutated through the bridge. It never exposes raw API forwarding, Herdr server start/stop/restart, subscriptions, focus or takeover, integrations, plugins, or the client's own task lifecycle. The client owns the MCP child and closing its stdin ends it without changing the Herdr server.
...
This is a same-user capability and product-policy boundary, not hardened authorization against the local account. Queue directories and opaque channel capabilities prevent accidental cross-channel access, and MCP mutation tools revalidate registered pane identity, but the existing Herdr UI, CLI, and full local socket API keep their normal authority for the same OS user. The bridge is therefore suitable for making a desktop agent the sole intended manual entry point, not for sandboxing a hostile local process.
