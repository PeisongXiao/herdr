...
For supported clients, prefer automatic setup:

```bash
herdr mcp install codex
herdr mcp status codex
herdr mcp uninstall codex
```

The installer supports Codex, Claude Code, Hermes, and OpenClaw. It writes the
fixed `herdr` entry in restricted mode unless `--full-control` is explicit.
OpenCode and other client IDs report `Currently unsupported`; use the manual
configuration below for them. Installing an entry does not create a connection
or turn the MCP indicator green. That happens only when a client starts the
configured stdio bridge.


On macOS and Linux, any MCP-capable coding agent can start Herdr's local STDIO MCP bridge as a child process. The bridge controls a selected Herdr session but never appears as a Herdr session, pane, or agent itself. Both entry points are intentionally hidden from Herdr help, completions, menus, and integration lists: `herdr --session NAME mcp serve` runs the bridge, while a CLI launched by that bridge can use `herdr desktop enqueue` to append a message for later desktop inspection. The client still shows the configured MCP server in its own settings UI.

Register the bridge with your agent's MCP configuration, using the absolute path to the Herdr binary. For Codex desktop, edit the shared Codex configuration:
...
Clients that use JSON MCP configuration, such as Claude Code, Claude Desktop, or Cursor, register the same command and arguments:
...
This configuration uses restricted mode, which remains the default. To grant the bridge human-equivalent control over shared Herdr session and runtime operations, append `--full-control`:

```toml
args = ["--session", "default", "mcp", "serve", "--full-control"]
```

Restricted mode exposes exactly the ten tools listed in the first configuration. Health, snapshot, pane read, and process inspection are read-only. Launch, send, interrupt, and stop apply only to CLIs launched and registered by this MCP process; arbitrary panes remain inspectable but cannot be mutated through the bridge.

Full-control mode preserves those tools and adds this typed surface:

| Tool | Operations |
| --- | --- |
| `herdr_inspect` | List, inspect, read, and explain workspaces, tabs, panes, layouts, agents, worktrees, peers, remote agents, and terminal recovery. |
| `herdr_wait` | Wait for a typed session event or pane-output match. |
| `herdr_workspace_control` | Create, focus, rename, and move workspaces. |
| `herdr_tab_control` | Create, focus, rename, and move tabs. |
| `herdr_pane_control` | Split, swap, move, zoom, focus, directionally focus, resize, rename, and send pane input. |
| `herdr_layout_control` | Apply layouts and set split ratios. |
| `herdr_agent_control` | Send to, rename, focus, and start agents. |
| `herdr_worktree_control` | Create and open worktrees. |
| `herdr_peer_control` | Connect or disconnect SSH peers and send to, rename, or start remote agents. |
| `herdr_recovery_control` | Retry terminal recovery. |
| `herdr_destructive_action` | Close workspaces, tabs, and panes; remove worktrees and peers; discard terminal recovery. |

Inspect and wait tools are marked read-only, ordinary controls are marked non-destructive writes, and destructive operations are isolated in `herdr_destructive_action`. `herdr_wait` defaults to 60 seconds and accepts at most 10 minutes. `herdr_health` reports the bridge's `access_mode`.

Neither mode exposes raw API forwarding, Herdr server lifecycle, internal reporting and delegation methods, integrations, plugins, updates, configuration, or the client's own task lifecycle. The client owns the MCP child and closing its stdin ends it without changing the Herdr server.

The sidebar shows the bridge's server lease without creating a Herdr resource: gray `○ MCP` means no initialized bridge is connected, green `● MCP` means at least one restricted bridge is connected, and peach `● MCP` means at least one full-control bridge is connected. Full control takes precedence when both modes are present. The compact and mobile layouts show only the dot. A clean disconnect removes the lease immediately; heartbeats let the server clear the indicator after an unclean exit.

Each launched CLI receives a capability-scoped, local queue through reserved `HERDR_DESKTOP_*` environment variables. `herdr desktop enqueue` can only append bounded UTF-8 messages to that queue; it has no Herdr API client and cannot control its process, pane, server, or MCP lifecycle. Messages persist while the desktop client, the MCP child, or Herdr is stopped. `herdr_drain_messages` atomically leases a FIFO batch instead of deleting it, and `herdr_ack_messages` explicitly and idempotently removes the leased messages after the client has handled them. An unacknowledged expired lease returns to the queue, so inspection does not silently lose messages.

The enqueue command accepts exactly one message source:
...
Messages are UTF-8 and at most 64 KiB; `kind` defaults to `info`. One channel retains at most 1,000 messages or 8 MiB, and one session retains at most 32 channels. Drain leases default to 20 messages, accept at most 100, and never return more than 1 MiB. A lease expires after 10 minutes and returns its unacknowledged messages to FIFO order. A successful acknowledgment keeps a 24-hour idempotency receipt.

This is a same-user capability and product-policy boundary, not hardened authorization against the local account. Queue directories and opaque channel capabilities prevent accidental cross-channel access, and MCP mutation tools revalidate resource identity, but the existing Herdr UI, CLI, and full local socket API keep their normal authority for the same OS user. Adding `--full-control` is an explicit authority grant to the configured MCP client. The bridge is suitable for making a desktop agent the sole intended manual entry point, not for sandboxing a hostile local process.
...
For authenticated SSH commands, `herdr api bridge` reads an initial JSON
request from stdin, forwards it to the local full API socket, and streams the
response to stdout. Federation uses this helper for setup and remote-agent
status projection. The long-lived peer link does not expose the full API socket.
...
- start and exclusively present an agent through an SSH transport by passing `{"transport":{"type":"ssh","target":"host","session":"default"}}` to `agent.start`
- connect an interactive SSH peer shell with `peer.connect_ssh`, delegate its pane to one presentation owner, and register, list, release, unregister, or health-check the runtime peer link
...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send`, `agent.rename`, `agent.focus`, `agent.start`, `agent.attach.prepare` |
| Terminal delegation | `terminal.delegate.create`, `terminal.delegate.claim`, `terminal.delegate.status`, `terminal.delegate.terminate`, `terminal.delegate.handoff` |
...
Some CLI commands are conveniences around these methods. For example, `herdr agent wait` resolves an agent target and then subscribes to pane agent state events or polls a peer target.

The socket API keeps an omitted `include_peers` field local-only for compatibility. Pass `{"include_peers":true}` to include reachable registered peers; the `herdr agent list` CLI does this explicitly. Peer-aware agent targets use `peer::target`. The server translates supported agent operations to the corresponding `peer.agent.*` method, preventing recursive forwarding through a peer.

`peer.connect_ssh` is the server-side half of pane `ssh TARGET` integration. It ensures that a compatible remote Herdr daemon is reachable, requires matching protocol plus advertised `peer_federation` and `remote_presentation` capabilities, registers both runtime peers, starts a reverse SSH forwarding bridge to the restricted peer socket, and prepares a new remote terminal for exclusive delegation. The response contains a connection id and attach data carrying the delegation claim. The built-in pane wrapper and `agent start --ssh` pass `session: "default"` explicitly, preventing inherited or SSH-accepted Herdr session and socket variables from redirecting remote commands. The server also normalizes an omitted or empty API session to `default`; pass a non-empty name only when a named remote session is intended. The server rejects effective SSH config that changes interactive session semantics before starting managed federation work, including for direct `agent.start` and `peer.connect_ssh` API requests.

The caller identifies the presentation owner with `owner_pane_id`; nested calls inherit the existing `owner` and extend its route instead of creating another presentation. The remote terminal is removed from its host workspace when the terminal-stream client atomically commits the claim, so the pane appears in a workspace on exactly one machine. `agent.attach.prepare` uses `terminal.delegate.claim` for an existing peer agent. A claim against an already presented pane fails unless `takeover` is true; takeover replaces the previous owner rather than duplicating the pane. A pane that currently owns a deeper remote presentation must hand back or close that deeper presentation before it can itself be taken over.

The owner must renew a pane connection with `peer.keepalive_ssh`; the built-in wrapper does so every three seconds. The server expires an unrenewed wrapper connection after 15 seconds, and the CLI calls `peer.disconnect_ssh` when the attach exits normally. A prepared connection is non-destructive until the remote attach is observed as active, so cancellation during setup preserves the local pane. A peer-agent attach reuses the peer for routing but receives a separate 15-second owner-presentation lease renewed by successful mirror snapshots; closing that owner never removes an unrelated shared bridge. The delegated attach is destructive after activation: disconnect, owner-pane closure, owner-lease expiry, or owner-server stop terminates the remote terminal unless `terminal.delegate.handoff` has already restored it to a new workspace on its host. An incoming reverse `peer.register` has a separate 15-second lease, renewed only by a successful metadata refresh. Origin crashes and network loss therefore remove ghost peers and terminate presentations whose ownership route crossed the expired peer. Every bridge conditionally unregisters only when the current remote registration still names its own forwarded socket, so concurrent bridge cleanup cannot erase a replacement. Socket and control cleanup runs detached, and server live handoff remains blocked while a delegation, presentation, peer link, or pending cleanup exists. Peer registration is runtime state rather than a durable trust or pairing record.

The delegation methods form an owner-checked lifecycle. `terminal.delegate.create` prepares a fresh shell, while `terminal.delegate.claim` prepares an existing pane. A fresh shell may include the SSH bridge's current directory and environment; Herdr strips its own routing variables before launch. Remote agent starts mark their uncommitted existing-pane claims for destructive expiry so a failed final attach cannot leave an agent running invisibly. The direct attach client commits the opaque delegation id and epoch when it takes ownership. `terminal.delegate.status` reports `pending`, `active`, `taken_over`, `handed_off`, `terminated`, or pre-commit `failed`. `terminal.delegate.terminate` abandons a pending claim or destroys an active delegated terminal. `terminal.delegate.handoff` accepts the delegated pane's own public `pane_id`, removes its delegation, and creates a new workspace around that live pane on the host server. The CLI exposes only the cursor-scoped form as `herdr remote-handoff`; it takes no target and cannot hand off an arbitrary pane.

The restricted listener permits `ping`, `peer.agent.list`, `peer.agent.get`, and `peer.presentation.activate`. The last method carries only a delegation id and epoch so the hosting server can acknowledge the atomic attach commit to the immediately preceding ownership hop. It rejects `peer.agent.explain`, terminal and detection-preview reads, input, rename, process launch, normal workspace/tab/pane/event operations, integrations, plugins, general peer management, and server control with `method_not_allowed`. This narrow metadata-and-lifecycle boundary lets both servers discover agent state and close activation races without granting the SSH destination command authority on the initiating machine. The SSH reverse forward never targets the normal full JSON API socket or Herdr's private binary client socket. `herdr api bridge` still reaches the normal local API, but only as an explicitly invoked command inside the authenticated SSH session; it is not the forwarded peer endpoint.

Creating the reverse bridge requires the SSH daemon and any applicable `authorized_keys` restrictions to permit remote Unix-socket forwarding. A denial fails explicit `agent.start` or `peer.connect_ssh` with a `remote peer bridge failed` error that retains the actionable remote Unix-socket forwarding and server or authorized-key policy diagnostic; the API does not return a partially connected peer or claim one-way awareness. The pane shim prints that same server error before invoking the real `ssh` with its original raw argv.

The same split applies to remote discovery and capability errors. Raw `peer.connect_ssh` and SSH-backed `agent.start` return an error when remote Herdr is missing, its protocol differs, or either required capability is absent. The interactive pane wrapper prints that error as a fallback warning, cleans up the managed connection, and invokes ordinary SSH with the user's original argv.
...
On Unix, servers also create a restricted peer listener for federation. Each
socket endpoint uses mode `0600`. The peer listener has a compact opaque name
derived from process- and startup-specific input, normally in the full API
socket's directory. If that path would exceed portable Unix-socket limits,
Herdr uses a compact temporary-directory fallback. A stale reverse forward
therefore cannot reconnect to a replacement server after restart.
...
These are the normal API paths. The restricted federation listener is an
internal per-server endpoint and intentionally has no stable filename or fixed
relationship to `herdr.sock`; its compact fallback may be in a temporary
directory. Its identity changes when the server restarts, so clients must not
derive, cache, or connect to it directly. Herdr's peer setup passes the current
endpoint internally. Normal scripts and plugins should continue to use
`herdr.sock` through CLI wrappers or the raw API.
...
An SSH-backed local presentation may include `mirror_of_terminal_id`. This is a metadata-only identity hint used to suppress a duplicate peer listing when the owner already projects the same native terminal; it does not expose a transport, attach command, session reference, or terminal contents.
...
`server.agent_manifests` returns the active bundled or local agent detection manifest sources without reloading rules:
...
The optional `active_version` and `warning` fields are omitted when not available. `server.reload_agent_manifests` returns `agent_manifest_reload` with the same `manifests` item shape after reloading the in-memory rule cache.
